...
public class Player{
	...
	public static List<Player> all(){
		try(Connection con = DB.sql2o.open()){
			String sql = "SELECT id, username AS userName, password FROM player";
	...
	public static Player find(String userName, String password){
		try(Connection con = DB.sql2o.open()){
			String sql = "SELECT id, username AS userName, password FROM player WHERE username = :userName AND password = :password";
	...
	public static Player find(int searchId){
		try(Connection con = DB.sql2o.open()){
			String sql = "SELECT id, username AS userName, password FROM player WHERE id = :searchId";
			return con.createQuery(sql)
